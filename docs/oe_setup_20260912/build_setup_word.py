...
SRC = Path('/tmp/oe_setup_20260912/oe_experimental_setup_and_evaluation_20260912.md')
OUT = Path('/tmp/oe_setup_20260912/oe_experimental_setup_and_evaluation_20260912.docx')
...
def mm(n):
    ...
    if tag in ('mi','mn','mo','mtext'):
        if txt in ('\u2061','\u2062'):return []
        return mr(txt,n.get('mathvariant'),normal=(tag in ('mtext','mn','mo') or n.get('mathvariant')=='normal' or (len(txt)>1 and not n.get('mathvariant'))))
    if tag=='mspace':
        width=n.get('width','0')
        if width.startswith('-'):return []
        return mr('\u2003' if width.startswith('2') else '\u2009',normal=True)
    if tag=='msqrt':
        a=el('m:rad');pr=add(a,'m:radPr');add(pr,'m:degHide',val='1');add(a,'m:deg');e=add(a,'m:e')
        for child in kids:into(e,child)
        return [a]
    if tag=='mfrac':
    ...
    if tag=='mover':
        ...
        if marker in ('^','ˆ','̂','¯','ˉ','→','˜','~'):
            a=el('m:acc');pr=add(a,'m:accPr');add(pr,'m:chr',val={'^':'\u0302','ˆ':'\u0302','~':'\u0303','˜':'\u0303','¯':'\u0304','ˉ':'\u0304'}.get(marker,marker))
            into(add(a,'m:e'),kids[0]);return [a]
        a=el('m:limUpp');into(add(a,'m:e'),kids[0]);into(add(a,'m:lim'),kids[1]);return [a]
    if tag=='munderover':
        a=el('m:limUpp');lower=add(add(a,'m:e'),'m:limLow')
        into(add(lower,'m:e'),kids[0]);into(add(lower,'m:lim'),kids[1]);into(add(a,'m:lim'),kids[2]);return [a]
    if tag=='munder':
...
def paragraph(node,style='Normal',parent=body,keep=False):
    p=add(parent,'w:p');pp=props(p,style,keep=keep)
    if style=='Normal':add(pp,'w:keepLines')
...
def table(node):
    ...
    borders=pr.find('{%s}tblBorders'%NS['w'])
    for side in ['left','right','insideH','insideV']:borders.find('{%s}%s'%(NS['w'],side)).set('{%s}val'%NS['w'],'nil')
    ...
    widths={2:[1100,8260],4:[1350,2350,3210,2450],5:[1150,1800,2450,1960,2000]}.get(ncols,[9360//ncols]*ncols)
    grid=add(t,'w:tblGrid')
    ...
    for ri,row in enumerate(rows):
        ...
        for ci,cell in enumerate(row):
            ...
            if ri==0:
                add(cp,'w:shd',fill='E8EEF4',val='clear')
                add(add(cp,'w:tcBorders'),'w:bottom',val='single',sz=4,color='A0A8B0')
            p=add(tc,'w:p');props(p,'TableText',align='left')
...
st=style('TableText',19,after=30);st.find('{%s}pPr'%NS['w']).find('{%s}spacing'%NS['w']).set('{%s}line'%NS['w'],'240')
...
wr(p,'Optics Express  |  Experimental setup and evaluation  |  12 September 2026')
footer=E.Element('{%s}ftr'%NS['w'],nsmap={'w':NS['w']});p=add(footer,'w:p');props(p,'Running',align='right')
wr(p,'Experimental setup and evaluation  |  ');f=add(p,'w:fldSimple',instr='PAGE');wr(f,'1')
...
Path('/tmp/oe_setup_20260912/conversion_report.json').write_text(json.dumps(report,indent=2))
