function report = priority_validation_loss_target_task(repoRoot, outputRoot, sampleId, repeatIndex)
%PRIORITY_VALIDATION_LOSS_TARGET_TASK Independent 90-frame loss targets.
...
prepared=load(fullfile(dataRoot,sampleId,'prepared.mat'),'ground_truth');
illuminationPath=fullfile(outputRoot,'shared_illumination',sprintf('repeat_%02d.mat',repeatIndex));
assert(isfile(illuminationPath),'priority:MissingIllumination');
...
for frame=1:100
    ...
    if frame<=10
        input(:,:,frame)=double(sensor);
    else
        holdout(:,:,frame-10)=double(sensor);
    end
end
...
report=struct('complete',true,'sample_id',sampleId,'repeat_index',repeatIndex, ...
    'holdout_frames',90,'output_mat',outputPath);
