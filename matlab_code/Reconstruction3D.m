...
function [] = Reconstruction3D(H,Ht,CAindex,inputFilePath,inputFileName,savePath,it,depth_z1,depth_z2)
warning('off');
addpath('./Util/');
addpath('./Solver/');
eqtol = 1e-10;
...
%     LFmovie = LFmovie(55*3+1:55*16,55*3+1:55*16);
    FirstFrame = 1;
    LastFrame = 1;
    DecimationRatio = 1;
...
%%%%%%%%%%%%%%%%%% Save the results %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
imwrite( squeeze(XguessSAVE1(:,:,1)), [savePath 'Recon3D_' inputFileName(1:end-4) '_depth_from',num2str(depth_z1),'to',num2str(depth_z2),'.tif']);
for k = 2:size(XguessSAVE1,3),
    imwrite(squeeze(XguessSAVE1(:,:,k)),  [savePath 'Recon3D_' inputFileName(1:end-4) '_depth_from',num2str(depth_z1),'to',num2str(depth_z2),'.tif'], 'WriteMode', 'append');
end
if saturate,
